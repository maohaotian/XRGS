...
def readCamerasFromTransforms(path, transformsfile, white_background,objects_folder, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            image_path =  cam_name # why??? still wrong here
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1],objects=objects))
            
    return cam_infos

def readNerfSyntheticInfo(path, white_background, eval,object_path, extension=".png"):
    object_dir = 'object_mask' if object_path == None else object_path
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension,os.path.join(path, object_dir))
