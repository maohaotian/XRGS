...
from utils.loss_utils import l1_loss, ssim, loss_cls_3d,aniso_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        an_loss = aniso_loss(gaussians) # add anisotropic loss

        loss_obj_3d = None
        if iteration % opt.reg3d_interval == 0 and iteration <= opt.reg3d_limit:
            ...
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + loss_obj + loss_obj_3d +an_loss
        else:
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + loss_obj+an_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1_000,2000,3000,4000,5000,7_000, 30_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1_000,2000,3000,4000,5000, 7_000, 30_000, 60_000])
    ...
    args.reg3d_limit = config.get("reg3d_limit", 3500)
