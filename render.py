...
# not sure should we put all masks in one image. now put them in one image
def obj2mask_binary(objects,ids):
    mask_binary = np.zeros((objects.shape[-2:]), dtype=np.uint8)
    for id in ids:
        mask_binary[objects == id] = 255 # mask part is set to 255
    return mask_binary
